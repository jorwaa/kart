...
function App() {
  ...
  const [existingMapOptions, setExistingMapOptions] = useState<{center: {lat: number, lng: number}, zoom: number, mapTypeId: string} | null>(null);
  ...
  return (
    ...
    {checkins?.activeDays.map((date: string, i: number) => (
      ...
      <GoogleMaps2 options={existingMapOptions} setOptions={setExistingMapOptions}  markers={[...markers.markers, ... markers.latest ? [markers.latest] : []]} center={(markers.latest ? { lat: markers.latest.lat, lng: markers.latest.lng } : null)} loader={loader} />
  </>
  )
};
